 ...
 *
 * No DOM, no I/O. Usable in the browser (attaches to window.FusionEngine)
 * and in Node (module.exports) for testing.
 */
(function (root, factory) {
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = factory(require('../shared/parallels.js'));
  } else {
    root.FusionEngine = factory(root.Parallels);
  }
})(typeof self !== 'undefined' ? self : this, function (Parallels) {
  'use strict';
  ...
  const MATCH_SYNERGY = { 0: null, 1: 0, 2: 2, 3: 5, 4: 8, 5: 12, 6: 12 };
  ...
  const DIGITAL_PARALLELS = Parallels.DIGITAL_ORDER.slice();
  const PHYSICAL_PARALLELS = Object.keys(PHYSICAL_PARALLEL_BONUS);

  function normalize(s) {
    return (s == null ? '' : String(s)).trim().toLowerCase();
  }

  function isPhysicalParallel(parallel) {
   ...
   */
  function countMatches(card, fusion, opts) {
  ...
  function parallelBonus(parallel) {
   ...
   */
  function cardVariants(card, fusion, availableParallels) {
   ...
   */
  function parseRequirement(str, fusion) {
    const raw = (str || '').trim();
   ...
   */
  function formatRequirement(str, fusion) {
    const raw = (str || '').trim();
    if (!raw) return 'Any';
   ...
   */
  function enumerateCombinations(domain, slots, target, cap) {
   ...
   */
  function checkFeasibility(combo, availabilityByValue) {
    const need = {};
  ...
  return {
    MATCH_SYNERGY,
    DIGITAL_PARALLEL_BONUS,
    PHYSICAL_PARALLEL_BONUS,
    TIERS,
    DIGITAL_PARALLELS,
    PHYSICAL_PARALLELS,
    normalize,
